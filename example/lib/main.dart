...
class MyAppState extends State<MyApp> with TickerProviderStateMixin {
  ...
  final ValueNotifier<List<SvgColorElement>> svgColorElements =
      ValueNotifier([]);
  List<ElementEdit> partsQuerySelector = [];
  ...
  Map<String, dynamic> apiModifications = {
    "elements": [
      {
  ...
  // Parse API data to a list of ElementSvg objects
  List<ElementEdit> parseModifications(Map<String, dynamic> data) {
    List<ElementEdit> modifications = [];
    for (var element in data["elements"]) {
      modifications.add(ElementEdit(
        querySelector: element["querySelector"],
        fillColor: Color(element["fillColor"]),
      ));
    }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          Expanded(
            ...
            child: Center(
              child: Column(
                ...
                children: [
                  // Animated SVG editor using animation controller
                  AnimatedBuilder(
                    animation: _animation,
                    builder: (context, child) {
                      return SvgPicEditor.asset(
                        "assets/test.svg",
                        width: 200,
                        height: 200,
                        fit: BoxFit.cover,
                        ...
                        modifications: [
                          ElementEdit(
                            querySelector: "#bulbo",
                            fillColor: _animation.value ?? Colors.yellow,
                            opacity: 0.5,
                          ),
                          ElementEdit(
                            querySelector: "path[id='eyes']",
                            fillColor: _animation.value ?? Colors.blue,
                            strokeWidth: 3,
                            strokeColor: Colors.black,
                          ),
                          ElementEdit(
                            querySelector: 'path[fill="#006359"]',
                            fillColor: _animation.value ?? Colors.blue,
                          ),
                        ],
                      );
                  ...
                  // String-based SVG editor
                  SvgPicEditor.string(
                    svgString(),
                    width: 200,
                    height: 200,
                    fit: BoxFit.cover,
                    modifications: partsQuerySelector,
                    getColors: (colors) {
                      svgColorElements.value.clear();
                      svgColorElements.value.addAll(colors);
                      setState(() {});
                    },
                  ),
                ],
          ...
          // Grid with SVG part colors
          Expanded(
            flex: 1,
            child: ValueListenableBuilder<List<SvgColorElement>>(
              valueListenable: svgColorElements,
              builder: (context, value, child) {
                return Column(
                  children: [
                    const Text("Parts colors"),
                    Container(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            onTap: () {
                              setState(() {
                                partsQuerySelector =
                                    value[index].parts.map((e) {
                                  return e.elementSvg.copyWith(
